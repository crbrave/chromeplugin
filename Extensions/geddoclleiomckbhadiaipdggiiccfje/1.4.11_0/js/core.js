...
//chrome.storage.local.remove(['hideKickstarter'])
var isHideKickstarter = false



function setHideKickstarter() {
  
  return function() {

  
  chrome.storage.local.set({
    "hideKickstarter": true
  }, function () {
    console.log("hide kickstarter");
    isHideKickstarter = true
    createContextMenus()
  }) 
...
chrome.contextMenus.removeAll(function() {
  ...
  chrome.storage.local.get(['hideKickstarter'], function (data) {
    let value = data['hideKickstarter'] && data['hideKickstarter'].newValue ? data['hideKickstarter'].newValue : data['hideKickstarter']
    isHideKickstarter = value || false


  let endDate = new Date(2019, 0, 24, 23, 59, 0, 0)
  let now = new Date()
  
  let remainingHours = (endDate.getTime() - now.getTime()) / 36e5;
  /*var oneDay = 24*60*60*1000; // hours*minutes*seconds*milliseconds
  let remainingDays = Math.round((endDate.getTime() - )/(oneDay));*/
  let hoursText = parseInt(remainingHours, 10) + ' hours'

 

    
  if (!isHideKickstarter && remainingHours >= 0) {
    
    var contextMenuId2 = chrome.contextMenus.create({
      "title": `⏰ ${hoursText} to support next release on KickStarter! 🡆`,
      "type": "normal",
      "contexts": ["browser_action"],
      "onclick": openKickStarter()
    });
    var contextMenuId3 = chrome.contextMenus.create({
      "title": `🗙 Hide the crowdfunding campaign Ad`,
      "type": "normal",
      "contexts": ["browser_action"],
      "onclick": setHideKickstarter()
    });
    clearInterval(interval)
    interval = setInterval(createContextMenus, getMinutesUntilNextHour() * 60 * 1000)
...
function setKickStarterBadge(delay = 0, notifiedByClick = false) {
  ...
  if (!isHideKickstarter && remainingHours >= 0) {
    ...
    interval2 = setInterval(setKickStarterBadge, getMinutesUntilNextHour() * 60 * 1000)
    let hours = parseInt(remainingHours, 10)
  ...
  //let daysOrLast = remainingDays > 0 ? `${remainingDays} days` : 'Last day' 
...
//new Date(license.createdTime).toLocaleDateString("en", {year: "numeric", month: "long", day: "numeric"})
  //if (!alreadyRun) {
    ...
    /*if (!alreadyNotifiedByClick && remainingHours >= 0) {
      ...
      timer = setTimeout(() => {
        ...
        alreadyRun = true
        let space = 6
        let sentence = `   ...⚠⚠⚠    ${hoursText} and ~30% backed, last chance to support the next release on kickstarter!...   ❤️    `
        ...
        let i = -1;
          ...
          loading = setInterval(() => {
            ...
            chrome.browserAction.setBadgeBackgroundColor({color: '#AD2A1A'})

...
//                Autocomplete.show();
        ...
        findCurrent: function() {
            //background.log('App.findCurrent(' + this.getSearch() + ')');
            if (this.getSearch().length === 0) {
                this.showContainer('hello');
                searchdiv.find('em').html('');
                Autocomplete.hide();
...
//                Autocomplete.show();
                ...
                background.model.findCurrent(searchTerm, function(result, lastCall, foundPositions) { // callback
                    if (result == null) {

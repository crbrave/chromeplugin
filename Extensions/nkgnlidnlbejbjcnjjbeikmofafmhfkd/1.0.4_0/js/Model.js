...
function Model(db) {
    ...
    this.findAutocomplete = function(searchTerm, callback, errorCallback) {
        ...
        var lowerBoundKeyRange = IDBKeyRange.lowerBound(lowercaseString);
        ...
        cursor.onsuccess = function(e) {
            ...
            // starts with searched term
                ...
                // MY: uncomment this, maybe reason was PHP7 functions (old version)
                ...
                // }
                // New version
                termObj = new SearchTerm(innercursor.value.full_name);
                // termObj = new SearchTerm(innercursor.value.full_name, innercursor.value.class);
                termObj.setObject(innercursor.value);
                indexedDBResults.push(termObj);
                foundCount++;
                ...
                background.nacl.messageCallback = function(response) {
                    ...
                    // background.log('NaCl response converted', naclTerms);
    ...
    this.findCurrent = function(searchTerm, callback, errorCallback) {
        if (!(searchTerm instanceof SearchTerm)) {
            throw 'Invalid argument, `searchTerm` must be instance of SearchTerm class.';
        }

        // background.log('Model.findCurrent', '(', searchTerm.toString(), ')');
        ...
        var lowerBoundKeyRange = IDBKeyRange.lowerBound(lowercaseString);
        var cursor = store.openCursor(lowerBoundKeyRange);
        var indexedDBResult = null;

        cursor.onsuccess = function(e) {
            var innercursor = e.target.result;

            if (innercursor && innercursor.value.full_name.indexOf(lowercaseString) === 0) {
                // cursor item starts with searched term
                // MY: uncomment this, maybe reason was PHP7 functions
                termObj = new SearchTerm(innercursor.value.full_name);
                termObj.setObject(innercursor.value);
                indexedDBResult = termObj;
            }

            callback(indexedDBResult, true);
        };
        cursor.onerror = $.isFunction(errorCallback) ? errorCallback : indexedDB.onerror;
    ...
    this.findClassMethods = function(searchTerm, callback, errorCallback) {
        ...
        var store = _getStore();
        var lowerBoundKeyRange = IDBKeyRange.lowerBound(searchTerm.getClass(), false);
        var results = [ ];
